...
router.post('/', function(req, res, next){
  ...
  console.log("get拿到的：",req.query)
  console.log("post拿到的：",req.body)

  //校验必传参数
  if(!username || !password){
    res.send({err:1,msg:'username,password为必传参数a'});
    return;
  }
  ...
  // nikename = nikename || '系统给的'; //可借助第三方昵称生成库

  //icon 借助multer  -》 icon 使用用户传递或者默认icon
  if(req.files.length>0 ){

    //改名 整合路径 存到 icon  用户头像上传的文件的操作
    fs.renameSync(
      req.files[0].path,
        ...
        if(!err){
          ...
          if(result.length>0){
            ...
            collection.insertOne({
              username,password,email,follow,fans,time,tel
            },(err,result)=>{
              if(!err){
